...
from models.fanetplus import GenFANetPlus, ContextPath, FeatureAlign, \
    SelfAttn, Context, BiSeNetOutput, FeatureSelectionModule, ConvBNReLU, Upsample


class OFAFANetPlus(GenFANetPlus):
    def __init__(self,
        ...
        if backbone_option == 'basic':
            from supernets.ofa_basic import OFABasicBlockNets
        ...
        context = Context(FeatureSelectionModule(DynamicConvLayer(
            ...
            SelfAttn(out_nc, DynamicConvLayer(
                nc_32_list, [out_nc], kernel_size=1, stride=1, use_bn=True, act_func='relu'),
                     ConvBNReLU(in_chan=out_nc, out_chan=out_nc, ks=1, padding=0)))

        align16 = FeatureAlign(FeatureSelectionModule(DynamicConvLayer(
            nc_16_list, [out_nc], kernel_size=1, stride=1, use_bn=True, act_func='relu')),
            Upsample(factor=2, out_nc=out_nc), out_nc=out_nc)

        align8 = FeatureAlign(FeatureSelectionModule(DynamicConvLayer(
            nc_8_list, [out_nc], kernel_size=1, stride=1, use_bn=True, act_func='relu')),
            Upsample(factor=2, out_nc=out_nc), out_nc=out_nc)
        ...
        super(OFAFANetPlus, self).__init__(cp, context, align16, align8, conv_out32, conv_out16, conv_out,
    ...
    def set_active_subnet(self, d=None, e=None, w=None, **kwargs):
        ...
        self.context.attn.conv1.active_out_channel = self.out_nc
    ...
    def get_active_subnet(self, preserve_weight=True, output_aux=False):
        ...
        context.attn.conv1 = context.attn.conv1.get_active_subnet(nc_32, preserve_weight)
        ...
        subnet = GenFANetPlus(cp, context, align16, align8, out8=deepcopy(self.conv_out),
...
if __name__ == '__main__':
    ...
    #     width_mult_list=[0.65, 0.8, 1.0], feature_dim_list=[[80, 104, 128], [168, 208, 256], [336, 408, 512]])
    # ofa_network.cp.backbone.set_constraint([0, 1, 2], constraint_type='depth')
    # ofa_network.cp.backbone.set_constraint([0.2, 0.25, 0.35], constraint_type='expand_ratio')
    # ofa_network.cp.backbone.set_constraint([0, 1, 2], constraint_type='width_mult')
    ...
    ofa_network.cp.backbone.set_constraint([2, 3, 4], constraint_type='depth')
    ofa_network.cp.backbone.set_constraint([3, 4, 6], constraint_type='expand_ratio')
